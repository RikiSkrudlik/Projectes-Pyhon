...
def save_as_excel():
    if grouped is not None:
        save_folder = filedialog.askdirectory(title="Select Folder to Save Excel Files")
        if save_folder:
            # Iterate through each seller's data and save it to a separate Excel file
            for seller, data in grouped.iterrows():
                save_file = f"{save_folder}/{seller}.xlsx"
                data_df = pd.DataFrame(list(zip(data['POL_Nombre Completo'], data['POL_PNActual'], 
                                                 data['Ramo'], data['POL_FC'])),
                                        columns=['Customer', 'Prima', 'Ramo', 'Fecha'])
                data_df.to_excel(save_file, index=False)
